...
(function () {
    ...
    isAndroidChrome = /android/i.test(userAgent) && /chrome/i.test(userAgent);
    ...
    var manageTextarea = (function () {
        ...
        return function manageTextarea(el, opts) {
            var keydown = null;
            // Fix typing issue on Nexus 7 Chrome 
            // var keypress = null;
            ...
            function feedCurrentInput(element) {
                curInput = $(element).val();
                if (isAndroidChrome) {
                    var index = curInput.indexOf("#");
                    if (index >= 0) {
                        curInput = curInput.substr(index + 1);
                    } else {
                        // Deletion detected:
                        $(element).blur();
                        $(element).val("#");
                        ...
                        setTimeout(function() {
                            keyCallback('Backspace');
                        });

                        var $trick = $("<input type='text' value='deleted an input.' style='width: 1px; height: 1px;' />")
                            .appendTo($(element).parent()).focus();

                        setTimeout(function () {
                            $(element).selectRange($(element).val().length);
                            $(element).focus();
                            $trick.remove();
                        }, 100);
                    }
    ...
    function createRoot(jQ, root, textbox, editable) {
        ...
        if (isAndroidChrome) {
            textareaSpan = root.textarea = $('<span class="textarea"><input type="password" value="#" /></span>');
        }

        var textarea = textareaSpan.children();
        ...
        var textareaManager = manageTextarea(textarea, {
            ...
            text: function (text) {
                if (text == "#") {
                    // hash # is used to solve the deletion issue on Nexus 7 Chrome.
                    return;
                }

                if (text == " ") {
                    // Normally the space can't reach here, but on some devices like Nexus 7 Chrome, it can reach
                    var parent = cursor.parent;
                    // cursor is in root editable, continue default
                    if (parent === cursor.root) {
                        return;
                    } else {
                        parent.bubble('onKey', 'Tab');
                    }
                } else {
        ...
        switch (cmd) {
            ...
            case 'html':
                return this.html().replace(/ ?hasCursor|hasCursor /, '')
                    ...
                    .replace(/<span class="?textarea"?><input type="password" \/><\/span>/i, "");
            case 'write':
                if (arguments.length > 1)
                    return this.each(function () {
                        var blockId = $(this).attr(mqBlockId),
